...
class CSLMDataset(Dataset):
    def __init__(self, CSVPath, hparams, is_train:True, CSVPathMixup=None):
        self.data = pd.read_csv(CSVPath)
        self.hparams = hparams
        self.apply_mixup = hparams.apply_mixup
        if self.apply_mixup:
    ...
    def __getitem__(self, idx):
        if self.apply_mixup:
            data = self.data.sample(n= 1, random_state= 24)
            sentence = self.data.iloc[0, 0]
            mixup_data = self.mixup.sample(n= 1, random_state= 24)
            sentence_mixup = mixup_data.iloc[0,0]

            #convert label to one-hot
            label_num = self.labels2num[data.iloc[0, 1]]
